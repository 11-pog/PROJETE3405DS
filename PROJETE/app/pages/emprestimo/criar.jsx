  ...
  const sendLoanRequest = async () => {
    ...
    try {
      const requestData = {
        ...
        expected_return_date: formatDateForBackend(returnDate),
        request_type: requestType
      };

      await api.post('emprestimos/solicitar/', requestData);
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {ownerBooks.length === 0 ? (
        <Text style={styles.noBooks}>
          {chatPartner} não tem livros cadastrados
        </Text>
        ) : (
          ownerBooks.map((book) => (
            <TouchableOpacity
        ...
        <Text style={styles.alertaText}>
          ⚠️ ATENÇÃO:{"\n\n"}
          Para garantir uma experiência segura e agradável, siga estas
          orientações ao marcar o encontro:{"\n\n"}
          📍 Escolha sempre um local público e movimentado para realizar o
          encontro, como praças, cafés ou bibliotecas.{"\n"}
          👨‍👩‍👧 Se você for menor de idade, vá acompanhado de um responsável (pai,
          mãe ou outro adulto de confiança).{"\n"}
          📱 Avise alguém de confiança sobre o local e o horário do encontro.{"\n"}
          🚫 NUNCA compartilhe dados pessoais sensíveis com desconhecidos.{"\n\n"}
          Nosso objetivo é conectar leitores com segurança e responsabilidade.
          Boas trocas e boas leituras! 📚✨
        </Text>
        ...
        >
          <Text style={styles.createButtonText}>
            {loading
              ? 'Enviando...'
              : `Solicitar ${requestType === 'emprestimo' ? 'empréstimo' : 'troca'}`}
          </Text>
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
